...
On considère $H(z)$ une transformée en $z$ qui se présente sous la forme d'une fraction rationnelle en $z^{-1}$ (degré du numérateur strictement inférieur à celui du dénominateur) et on recherche la séquence temporelle $h(n)$ associée. On accèdera par exemple à la solution via une décomposition en éléments simples, sous la forme
...
$$H(z)=\sum_{k=1}^{K} \dfrac{A_k}{1-P_kz^{-1}}$$
...
Par extension, les pôles d'ordre supérieur peuvent aussi être traités. Un pôle $P_k$ d'ordre 2 est associé par exemple au terme ci-dessous dans $h(n)$.
...
Le changement de variable envisagé avec l'application de cette méthode est plutôt de la forme $y = 1-P_k x$ avec $x = z^{-1}$ afin de conserver le principe d'utiliser les expressions en $z^{-1}$ sous la forme

$$H(z)=\sum_{k=1}^{K} \sum_{m=1}^{M_k} \dfrac{A_{k,m}}{(1-P_kz^{-1})^m}$$
